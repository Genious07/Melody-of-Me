...
export default function Dashboard() {
  ...
  const handleGenerateClick = async () => {
    ...
    try {
      ...
      if (!bioRes.ok) {
        throw new Error(`Biography generation failed: ${bioRes.statusText}`);
      }
      ...
      <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
        <div className="mx-auto grid w-full max-w-6xl gap-6">
            ...
            {analysisData && (
                <div className="scroll-reveal">
                    ...
                    {analysisData.map((era, index) => (
                        <Card key={index} ref={el => eraCardsRef.current[index] = el} className="flex flex-col opacity-0">
                        <CardHeader>
            ...
            {!analysisData && !isLoading && (
             ...
             {isLoading && (
                 <div className="text-center p-8 flex flex-col items-center justify-center border-2 border-dashed border-muted-foreground/30 rounded-lg">
